...
def mask_attn(scores, actual_num_images, max_num_images, device):
    ...
    masks = torch.tensor(masks).unsqueeze(
        2).byte().to(device, dtype=torch.bool)
    scores.masked_fill_(~masks, float(-1e30))
    return scores
...
def evaluate(split_data_loader, model, epoch, args, timestamp, best_accuracy, best_loss, isValidation, weight):
    ...
    for ii, data in enumerate(split_data_loader):
        ...
        for p in range(preds.shape[0]):
            ...
            if torch.all(torch.eq(preds[p], ground_truth[p])):
                matching += 1

        for pi in range(preds.shape[0]):
            for pj in range(preds[pi].shape[0]):
                if ground_truth[pi][pj] == 0:
                    normalizer_0 += 1
                    if preds[pi][pj] == 0:
                        matching_0 += 1
...
def gold_evaluate(split_data_loader, criterion, threshold, weight):
    ...
    for sr in segment_rank_res:
        ...
        # nm1 predicted as 1 but actually 0
        sr_norm_0 = rank_res['matching_0'] + rank_res['non_match_1']
        ...
        print(sr_prec_0, sr_prec_1, sr_rec_0,
              sr_rec_1, sr_f1_0, sr_f1_1, sr_w_f1)
...
def gold_evaluate_write(split_data_loader, dataset, breaking, mask, img_dim, model, seg2ranks, id_list, device, criterion, threshold, weight):
    ...
    with open('hard_match_hist.txt', 'w') as file:
        ...
        with open('hard_miss_hist.txt', 'w') as miss_file:
            ...
            for ii, data in enumerate(split_data_loader):
                ...
                out = model(text_feature[0], img_features)
...
if __name__ == '__main__':
    ...
    timestamp = str(t.date()) + '-' + str(t.hour) + '-' + \
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            isValidation = False
            print('\nTrain Eval')
            evaluate(training_loader,
                     model, epoch, args, timestamp, best_accuracy, best_loss, isValidation, weight)

            isValidation = True
            print('\nVal Eval')

            best_accuracy, best_loss, current_f1, current_loss = evaluate(val_loader,
