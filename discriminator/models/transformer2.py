...
class HistoryModelBlind(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, img_dim, num_heads=4, num_layers=4, max_len=5000):
        super().__init__()
        ...
        self.positional_encoding = nn.Parameter(self.get_positional_encoding(self.max_len, self.embedding_dim), requires_grad=False)

        encoder_layer = nn.TransformerEncoderLayer(d_model=self.embedding_dim, nhead=self.num_heads, dim_feedforward=self.hidden_dim)
    ...
    def forward(self, segment, prev_histories, lengths, separate_images, visual_context, normalize, device):
        ...
        positional_encoding = self.positional_encoding[:, :seq_len, :].to(embeds_words.device)
        ...
        for b in range(batch_size):
            ...
            for p in range(len(prev_hist)):
                ...
                if len(prev_hist[p]) > 0:
                    ...
                    hist_positional_encoding = self.positional_encoding[:, :hist_len, :].to(hist_embeds.device)
