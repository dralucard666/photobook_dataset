...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        )  # Shape: (batch_size, seq_len, 3, embed_dim)
...
class VisionLanguageTransformer(nn.Module):
    def __init__(self, embed_dim=768, num_heads=8, N_blocks=2, device="cpu"):
        ...
        self.blocks = [
            SelfAttentionBlock(embed_dim, num_heads).to(device) for _ in range(N_blocks)
        ]
    ...
    def forward(self, text_embedding, image_embeddings):
        ...
        attn_output = attn_output.permute(1, 0, 2).squeeze(
            1
