...
model_files = ['model_blind_accs_2019-02-17-21-18-7.pkl',
               'model_history_blind_accs_2019-02-20-14-22-23.pkl',  'model_history_noimg_accs_2019-03-01-14-25-34.pkl']
...
for model_file in model_files:
    ...
    if model_name_abbrv[model_file] == 'No history':
        model, epoch, loss, accuracy, args_train = load_model(
            model_file, DiscriminatoryModelBlind, device)
        color_plot = 'blue'
        line_plot = '-'
        marker_plot = ''
        ...
        model, epoch, loss, accuracy, args_train = load_model(
            model_file, HistoryModelBlind, device)
        color_plot = 'red'
        line_plot = '--'
        marker_plot = ''
        ...
        model, epoch, loss, accuracy, args_train = load_model(
            model_file, HistoryModelBlindNoTarget, device)
        color_plot = 'green'
        line_plot = ':'
        marker_plot = ''
    ...
    test_hist_loader = torch.utils.data.DataLoader(
        testset_hist, **load_params_test_hist)
    ...
    with torch.no_grad():
        ...
        if model_name_abbrv[model_file] == 'No history':
            rank_p_1, rank_r_1, rank_p_0, rank_r_0, segment_rank_res = train_nohistory.gold_evaluate(
                test_loader, testset, breaking, normalize, mask, img_dim, model, seg2ranks, device, criterion, threshold, weight)

        elif model_name_abbrv[model_file] == 'History':
            rank_p_1, rank_r_1, rank_p_0, rank_r_0, segment_rank_res = train_history.gold_evaluate(
                test_hist_loader, testset_hist, breaking, normalize, mask, img_dim, model, seg2ranks, id_list, device, criterion, threshold, weight)

        elif model_name_abbrv[model_file] == 'No image':
            rank_p_1, rank_r_1, rank_p_0, rank_r_0, segment_rank_res = train_history_noimg.gold_evaluate(
                test_hist_loader, testset_hist, breaking, normalize, mask, img_dim, model, seg2ranks, id_list, device, criterion, threshold, weight)
    ...
    # including all ranks, starting from 1
    sg_rank_x = np.arange(1, len(ranks)+1)
    plt.subplot(121)
    plt.ylim(top=100, bottom=30)

    plt.plot(sg_rank_x, p_plot[model_file][0:6], label=model_name_abbrv[model_file],
             color=color_plot, linestyle=line_plot, marker=marker_plot)
