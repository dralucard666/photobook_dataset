...
def evaluate(split_data_loader, dataset, breaking, normalize, mask, img_dim,
    ...
    # WARNING needs to be changed for different datasets
...
if __name__ == '__main__':
    ...
    model = DiscriminatoryModelBlind(
        len(vocab), embedding_dim, hidden_dim, img_dim).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(training_loader):
            ...
            # for the last batch, in case it has a different size
            temp_batch_size = data['segment'].shape[0]

            context_sum = torch.zeros(temp_batch_size, img_dim).to(
                device)  # not used in this model
            ...
            for b in range(image_set.shape[0]):
                ...
                for i in range(image_set[b].shape[0]):
                    ...
                    if img_id != '0':
                        ...
                        img_features = torch.Tensor(
                            trainset.image_features[img_id]).to(device)
